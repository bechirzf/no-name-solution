    ...
    public static function checkToken($token)
    {
        try {
            ...
            if (!$token['party']) {
                ...
                if (!$token['party']) {
                    throw new \Exception('Invalid token or your account may have been disabled.', 401);
                }
            ...
            // $api_key = ApiKey::where($token['payload']['sub'],'api_key')->first();

            // if(!$api_key){
                // Cache::forget($token['payload']['sub']);
            //     throw new \Exception('Invalid token or your account may have been disabled.', 401);
            // }
